...
class TestCircuitInfo(BaseClass):
    def test_circuitinfo(self):
        ...
        get_timeplace_db = queries.get_shape % tpid  # 2020-11-07-V-01
        ...
        log.info("Data from Meteorology db is retrieved for the timeplaceid: " + str(tpid))
        ...
        log.info("-----------------------------------------------------------------------------------------------")
        hfratablefilename = filename_hfratable.split(s3config()['envpath'])[-1]
        ...
        splitfile = hfratablefilename.split('/')[-1]
        splitfile1 = hfratablefilename.split(splitfile)
        print(str(splitfile1[0]))
        hrfafile = hfra + "/" + str(splitfile1[0])
        ...
        arcpy.Intersect_analysis([metfilename, temp_hfrafeaturelayerpath],
                                 hfraintersectedlayerfolder + "\\intersectedlayer")
        log.info("Intersection done on Meterology tp and HFRA layer")

        # Get the latest ohconductor table filepath from db
        get_ohconductortable_db = queries.get_activetablename % 's3-ssd-oh-isolationzone'
        lst_ohconductortable_details = queryresults_fetchone(get_ohconductortable_db)
        filename_ohconductortable = lst_ohconductortable_details
        log.info("Filename for ohconductor active table is : " + str(filename_ohconductortable))
        log.info("-----------------------------------------------------------------------------------------------")
        ohconductortablefilename = filename_ohconductortable.split(s3config()['envpath'])[-1]
        print(ohconductortablefilename)

        # Download ohconductor parquet file from S3 bucket
        s3 = boto3.client('s3')
        s3_resource = boto3.resource("s3")
        s3_bucketname = s3config()['pspsdatabucketname']
        ohconductorBUCKET_PATH = ohconductortablefilename
        profilename = s3config()['profile_name']
        ohconductor = downloadsfolderPath + "\\ohconductor"
        deleteFolder(ohconductor)
        create_folder(ohconductor)
        download_dir_from_S3(ohconductorBUCKET_PATH, s3_bucketname, profilename, ohconductor)
        log.info("Downloaded ohconductor parquet file from S3")
        time.sleep(5)
        log.info("Starting to load OH Conductor file")
        ohconductorfile = ohconductor + "/" + ohconductortablefilename
        params = ImportTool().getParameterInfo()
        params[1].value = ohconductorfile
        ...
        # Do intersect on intersected HFRA layer and PriOHConductor layer
        ...
        # Get the latest feedernetwork table filepath from db
        get_feedernetworktable_db = queries.get_activetablename % 's3-feedernetworktrace'
        lst_feedernetworktable_details = queryresults_fetchone(get_feedernetworktable_db)
        filename_feedernetworktable = lst_feedernetworktable_details
        log.info("Filename for feedernetwork active table is : " + str(filename_feedernetworktable))
        log.info("-----------------------------------------------------------------------------------------------")
        feedernetworktablefilename = filename_feedernetworktable.split(s3config()['envpath'])[-1]
        print(feedernetworktablefilename)

        # Download feedernetwork parquet file from S3 bucket
        s3 = boto3.client('s3')
        s3_resource = boto3.resource("s3")
        s3_bucketname = s3config()['pspsdatabucketname']
        feedernetworkBUCKET_PATH = feedernetworktablefilename
        profilename = s3config()['profile_name']
        feedernetwork = downloadsfolderPath + "\\feedernetwork"
        deleteFolder(feedernetwork)
        create_folder(feedernetwork)
        download_dir_from_S3(feedernetworkBUCKET_PATH, s3_bucketname, profilename, feedernetwork)
        log.info("Downloaded feedernetwork parquet file from S3")
        time.sleep(5)

        log.info("Starting to load Feeder Network Table file")
        ffnsplitfile = feedernetworktablefilename.split('/')[-1]
        ffnsplitfile1 = feedernetworktablefilename.split(ffnsplitfile)
        print(str(ffnsplitfile1[0]))
        feedernetworkfile = feedernetwork + "/" + str(ffnsplitfile1[0])
        params = ImportTool().getParameterInfo()
        params[1].value = feedernetworkfile
        ...
        temp_ff_new = spark.read.parquet(feedernetworkfile)
        ...
        df_join_ssd_active_circuits = spark.sql(
            """SELECT s.circuitid, s.opnum, s.devicetype, s.circuitname, s.min_branch, s.max_branch, s.treelevel, s.order_num, s.substationname, s.consider, s.operable, s.OPERABLE_TRANS_PMETER_COUNT, v.STATUS FROM Temp_Join_FeedNWT s LEFT JOIN Temp_Ckt_Source v ON LPAD(s.circuitid,9,'0') = v.CIRCUITID AND v.STATUS = '5'""")
        ...
        df_join_ssd_active_circuits.coalesce(1).write.option("header", "true").format("csv").mode("overwrite").save(
            tempfolder_active_circuits)
        ...
        df_ckt_out_table = spark.read.csv(tempfolder_active_circuits + "/" + "Join_Feednwtable_Active_Ckts.csv",
                                          header=True)
        ...
        for i in range(len(list_unique_ckts)):
            if (list_unique_ckts[i]):
                new_df = pd.DataFrame(df_ckt_file.loc[((df_ckt_file['circuitid'] == list_unique_ckts[i]),
                ...
                all_df = pd.DataFrame(new_df.loc[(((new_df['min_branch'] == min_b) & (new_df['max_branch'] == max_b) & (
                            new_df['treelevel'] == min_t) & (new_df['order_num'] == max_o)),
                                                  ['circuitid', 'opnum', 'devicetype', 'circuitname', 'min_branch',
                                                   'max_branch', 'treelevel', 'order_num', 'substationname', 'consider',
                                                   'operable', 'OPERABLE_TRANS_PMETER_COUNT', 'STATUS'])])

                df_min = pd.DataFrame(new_df.loc[((new_df['min_branch'] == new_df['min_branch'].min()),
                                                  ['circuitid', 'opnum', 'devicetype', 'circuitname', 'min_branch',
                                                   'max_branch', 'treelevel', 'order_num', 'substationname', 'consider',
                                                   'operable', 'OPERABLE_TRANS_PMETER_COUNT', 'STATUS'])])
                if (df_min.shape[0] > 1):
                    ...
                    update_df_min = pd.DataFrame(df_min.loc[(((df_min['min_branch'] == mn_b) & (
                                df_min['max_branch'] == mx_b) & (df_min['treelevel'] == mn_t) & (
                                                                          df_min['order_num'] == mx_o)),
                                                             [
                                                                 'circuitid',
                                                                 'opnum',
                                                                 'devicetype',
                ...
                if (df_max.shape[0] > 1):
                    ...
                    update_df_max = pd.DataFrame(df_max.loc[(((df_max['min_branch'] == mn_brch) & (
                                df_max['max_branch'] == mx_brch) & (df_max['treelevel'] == mn_tree) & (
                                                                          df_max['order_num'] == mx_ornum)),
                                                             [
                                                                 'circuitid',
                                                                 'opnum',
                                                                 'devicetype',
                ...
                df_tree_level = pd.DataFrame(new_df.loc[((new_df['treelevel'] == new_df['treelevel'].min()),
                                                         ['circuitid', 'opnum', 'devicetype', 'circuitname',
                                                          'min_branch',
                                                          'max_branch', 'treelevel', 'order_num', 'substationname',
                                                          'consider', 'operable', 'OPERABLE_TRANS_PMETER_COUNT',
                                                          'STATUS'])])
                    ...
                    while m < length:
                        ...
                        if not (Remove_duplicates_Merge_DF.iloc[m].empty):
                            for n in range(new_df.shape[0]):
                                ...
                                if ((not ((new_df.iloc[n]['min_branch'] <= Merge_DF_sort_min_branch.iloc[m][
                                    'max_branch']) and (
                                                  new_df.iloc[n]['min_branch'] >= Merge_DF_sort_min_branch.iloc[m][
                                              'min_branch'])))):
                                    new_min_value = new_df.iloc[n]['min_branch']
                                    new_max_value = new_df.iloc[n]['max_branch']
                                    if ((new_min_value in sorted_min_branch) and (
                                            new_max_value in sorted_max_branch) and ((new_df.iloc[n]['min_branch'] <=
                                                                                      Merge_DF_sort_min_branch.iloc[m][
                                                                                          'max_branch']) and (
                                                                                             new_df.iloc[n][
                                                                                                 'min_branch'] >=
                                                                                             Merge_DF_sort_min_branch.iloc[
                                                                                                 m]['min_branch']))):
                                        new_df = pd.DataFrame(new_df.loc[((new_df['min_branch'] > new_min_value),
                                                                          ['circuitid', 'opnum', 'devicetype',
                                                                           'circuitname', 'min_branch', 'max_branch',
                                                                           'treelevel', 'order_num', 'substationname',
                                                                           'consider', 'operable',
                                                                           'OPERABLE_TRANS_PMETER_COUNT', 'STATUS'])])
                                        new_df_sorted = new_df.sort_values(by=['min_branch'])
                                        new_df = new_df_sorted
                                        log.info("New DF size is updated")
                                        break
                                    if not ((new_min_value in sorted_min_branch) and (
                                            new_max_value in sorted_max_branch) and ((new_df.iloc[n]['min_branch'] <=
                                                                                      Merge_DF_sort_min_branch.iloc[m][
                                                                                          'max_branch']) and (
                                                                                             new_df.iloc[n][
                                        ...
                                        Remove_duplicates_Merge_DF_new_df = pd.DataFrame(
                                            new_df.loc[((new_df['min_branch'] == new_min_value),
                                                        ['circuitid', 'opnum', 'devicetype',
                                        ...
                                        if (Remove_duplicates_Merge_DF_new_df.shape[0] > 1):
                                            ...
                                            update_Remove_duplicates_Merge_DF_new_df = pd.DataFrame(
                                                Remove_duplicates_Merge_DF_new_df.loc[(((
                                                                                                    Remove_duplicates_Merge_DF_new_df[
                                                                                                        'min_branch'] == min_brch) & (
                                                                                                    Remove_duplicates_Merge_DF_new_df[
                                                                                                        'max_branch'] == max_brch) & (
                                                                                                    Remove_duplicates_Merge_DF_new_df[
                                        ...
                                        Remove_duplicates_Merge_DF = pd.concat(
                                            [Remove_duplicates_Merge_DF, Remove_duplicates_Merge_DF_new_df])
                                        Remove_duplicates_Merge_DF = pd.DataFrame(
                                            Remove_duplicates_Merge_DF.drop_duplicates())
                                        Merge_DF_sort_min_branch = Remove_duplicates_Merge_DF.sort_values(
                                            by=['min_branch'])
        ...
        Remove_duplicates_df.to_csv(verify_output_file_ND,
                                    header=['ObjectID', 'circuitid', 'opnum', 'devicetype', 'circuitname', 'min_branch',
                                            'max_branch', 'treelevel', 'order_num', 'substationname',
        ...
        df_new_output_final_file = pd.DataFrame(df_new_output_final_file.loc[
                                                    ((df_new_output_final_file['opnum'] != 'CB'),
                                                     ['ObjectID', 'circuitid', 'opnum', 'devicetype',
                                                      'circuitname', 'min_branch',
                                                      'max_branch', 'treelevel', 'order_num',
                                                      'substationname', 'consider',
                                                      'operable',
        ...
        # add CB ckts
        df_CB = pd.DataFrame(
            df_ckt_file.loc[((df_ckt_file['opnum'] == 'CB'), ['ObjectID', 'circuitid', 'opnum', 'devicetype',
                                                              'circuitname', 'min_branch',
                                                              'max_branch', 'treelevel', 'order_num',
                                                              'substationname', 'consider',
                                                              'operable',
        ...
        df_new_output_final_file.to_csv(verify_output_file_ND,
                                        header=['ObjectID', 'circuitid', 'opnum', 'devicetype', 'circuitname',
                                                'min_branch',
                                                'max_branch', 'treelevel', 'order_num', 'substationname',
        ...
        df_new_output_final_file = df_new_output_final_file.drop(
            columns=['ObjectID', 'consider', 'operable', 'OPERABLE_TRANS_PMETER_COUNT'])
        ...
        for y in range((len(list_unique_ckts_expected)) - 1):
            cmp_df_ckt_final_file_expected = df_ckt_final_file.loc[
                (df_ckt_final_file['circuitid'] == list_unique_ckts_expected[y])]
            cmp_df_ckt_final_file_expected = cmp_df_ckt_final_file_expected.sort_values(by=['min_branch'])
            cmp_df_new_output_final_file_actual = df_new_output_final_file.loc[
                (df_new_output_final_file['circuitid'] == list_unique_ckts_expected[y])]
            ...
            if not list_unique_ckts_expected[y] in list_unique_ckts_actual:
                ...
                cmp_df_ckt_final_file_expected = cmp_df_ckt_final_file_expected.loc[
                    (cmp_df_ckt_final_file_expected['circuitid'] == list_unique_ckts_expected[y])]
                values = "Circuit Missing -->  " + str(list_unique_ckts_expected[y])
                cmp_df_ckt_final_file_expected = cmp_df_ckt_final_file_expected.replace(
                    to_replace=list_unique_ckts_expected[y], value=values)
                Mismatched_df = Mismatched_df.append(cmp_df_ckt_final_file_expected)
            rows, cols = np.where(values_compare == False)
            for item in zip(rows, cols):
                cmp_df_ckt_final_file_expected.iloc[item[0], item[1]] = ' {} --> {} '.format(
                    cmp_df_ckt_final_file_expected.iloc[item[0], item[1]],
                    cmp_df_new_output_final_file_actual.iloc[item[0], item[1]])
                Mismatched_df = Mismatched_df.append(cmp_df_ckt_final_file_expected)

        Mismatched_df.to_csv(mismatched_file, header=['circuitid', 'opnum', 'devicetype', 'circuitname',
                                                      'min_branch', 'max_branch', 'treelevel', 'order_num',
