...
class TestCircuitInfo(BaseClass):
    def test_circuitinfo(self):
        ...
        temp_fold1 = local_folder + "/reports/timeplacecreation/" + str(var_tp_id) + "/" + str(var_tp_uid) + "/meteorology/meteorology_" + str(var_tp_uid)
        print(temp_fold1)

        re_fn = os.listdir(temp_fold1)
        for file in re_fn:
            if fnmatch.fnmatch(file, '*.csv'):
                print(file)
                os.rename(temp_fold1 +'/'+ file, temp_fold1 + '/' + 'Final_Result.csv')
        ...
        log.info("Data from Meteorology db is retrieved for the scope version: " + str(tpid))
        ...
        ffnwfile = feederdevices + "/" + feederdevicestablefilename
        params = ImportTool().getParameterInfo()
        ...
        cnv_hfrapriohintersectedfolder = hfrapriohintersectedlayerfolder + "/" + "intersectedlayer.shp"
        newhfrapriohintersectedfolder = downloadsfolderPath + "/" + "hfrapriohintersectedtable"
        deleteFolder(newhfrapriohintersectedfolder)
        create_folder(newhfrapriohintersectedfolder)
        arcpy.TableToTable_conversion(cnv_hfrapriohintersectedfolder, newhfrapriohintersectedfolder, "ohlayerintrsct.csv")
        log.info("Converted the Intermediate meteorology shape file into CSV file successfully")
       ...
       # df_ohlayerintrscted = pd.read_csv(filepath_or_buffer= hfrapriohintersectedlayerfolder + "/" + "ohlayer1intrsct.csv", sep = '[$, |_]', engine = 'python')
        ...
        #     print(col)
        df_new_OH = spark.read.csv(newhfrapriohintersectedfolder + "/" + "ohlayerintrsct.csv", header=True)
        ...
        # tempfolder = r"C:\Hema_Workspace\gis-geomartcloud-pspsv4-automation-python\PSPSProject\downloads\Join_FNetworkTable_OH\joined_feeder_network_table"
        df_join_ssd.coalesce(1).write.option("header", "true").format("csv").mode("overwrite").save(tempfolder)
        df_join_ssd.cache()
        ...
        for file in filenames:
            if fnmatch.fnmatch(file, '*.csv'):
                # print(file)
                os.rename(tempfolder + '/' + file, tempfolder + '/' + 'joined_feeder_network_table.csv')

        # Get the latest circuit source table filepaths from db
        get_circuitsource_db = queries.get_activetablename % 's3-circuitsource'
        ckt_table_details = queryresults_fetchone(get_circuitsource_db)
        filename_cktsourcetable = ckt_table_details
        log.info("Filename for Circuit Source Active table is : " + str(filename_cktsourcetable))
        log.info("-----------------------------------------------------------------------------------------------")
        cktsourcestablefilename = filename_cktsourcetable.split(s3config()['envpath'])[-1]
        print(cktsourcestablefilename)

        # Download circuit source parquet file
        s3 = boto3.client('s3')
        s3_resource = boto3.resource("s3")
        s3_bucketname = s3config()['pspsdatabucketname']
        cktsourceBUCKET_PATH = cktsourcestablefilename
        profilename = s3config()['profile_name']
        cktsourcefolderfile = downloadsfolderPath + "\\cktsource"
        deleteFolder(cktsourcefolderfile)
        create_folder(cktsourcefolderfile)
        download_dir_from_S3(cktsourceBUCKET_PATH, s3_bucketname, profilename, cktsourcefolderfile)
        log.info("Downloaded Circuit Source parquet file from S3")

        cktsourcefolder = cktsourcefolderfile + "/" + cktsourcestablefilename
        ...
        file_names = os.listdir(tempfolder_active_circuits)
        for file in file_names:
            if fnmatch.fnmatch(file, '*.csv'):
                # print(file)
                os.rename(tempfolder_active_circuits + '/' + file, tempfolder_active_circuits + '/' + 'Join_Feednwtable_Active_Ckts.csv')
        ...
        df_ckt_final_file = pd.read_csv(temp_fold1 + "/" + "Final_Result.csv")
        ...
        New_remove_duplicates_df = pd.read_csv(verify_output_file, header=None)
        ...
        df_new_output_final_file = pd.DataFrame(df_new_output_final_file.drop_duplicates())
        ...
        print(len(list_unique_ckts_expected))
        ...
        print (len(list_unique_ckts_actual))
        ...
        for y in range ((len(list_unique_ckts_expected))-1):
            ...
            if not list_unique_ckts_expected[y] in list_unique_ckts_actual:
                ...
                log.info("Missing Circuit: " + str(list_unique_ckts_expected[y]))
        ...
        if ((df_ckt_final_file.shape[0]) - (df_new_output_final_file.shape[0]) == 0):
            log.info("No Circuit Mismatch Found")

        # print(Mismatched_df.shape[0])
        if ((Mismatched_df.shape[0]) != 0):
            Mismatched_df.to_csv(mismatched_file, header=['circuitid', 'opnum', 'devicetype', 'circuitname', 'min_branch', 'max_branch', 'treelevel', 'order_num', 'substationname'], index=False)
            mismatch_count = (df_ckt_final_file.shape[0]) - (df_new_output_final_file.shape[0])
            log.info("Circuit Mismatch Found")
            log.info(str(mismatch_count) + " : Circuits mismatch found")

         # Remove_duplicates_df_new = pd.read_csv(verify_output_file_ND)
        log.info("Completed the stage of Comparing two files")
        log.info("----------------------------------------------------------------------------------------------")
