...
class TestDefaultManagementPositive(BaseClass):
    def test_create_time_place(self):
        ...
        var_uploadFileName = "manageTempGen_Invalid_Circuits.csv"
        var_error_message = "One or more columns are missing in the file"
        eventpage.tp_validatefile_message(testDatafolderPath, var_uploadFileName, var_error_message)
        log.info("Validate File failure message")

        log.info("Circuit error validation for the second flle")

        homepage.reload_Home()
        homepage.navigate_manageTempGen()
        log.info("Select Manage Template Gen menu")
        var_flag = uielements.iselementEnabled(locators.new_timeplace_browse_button)
        if var_flag:
        ...
        var_uploadFileName = "manageTempGen_Invalid_Circuits2.txt"
        var_error_message = "Invalid File! Allowed file is a csv file."
        eventpage.tp_validatefile_message(testDatafolderPath, var_uploadFileName, var_error_message)
        log.info("Validate File failure message")
